...
function setup() {
    let canvas = createCanvas(Math.max(650, windowWidth*0.88), Math.max(800, windowHeight * 1.2));

    frameRate(60);

    sizeAreas();

    healedColor = color(10, 150, 20);

    infectedColor = color(230,0,0);

    colorS = color(50, 100, 180);

    colorE = color(120, 0, 0);

    colorI = infectedColor;
...
const drawSeirValues = () => {
    ...
    stroke(colorS)
    for (let t = 1; t < context.S.length; t++) {

        x = leftX + t * xDens;
        let xPrev = leftX + (t-1) * xDens;
        let h = yDensSeir * context.S[t];
        let hPrev = yDensSeir * context.S[t-1];
        line(xPrev, y - hPrev, x, y - h)
        circle(x, y - h, 2, 2);
    ...
    stroke(colorI)
    for (let t = 1; t < context.I.length; t++) {

        x = leftX + t * xDens;
        let h = yDensSeir * context.I[t];
        let xPrev = leftX + (t-1) * xDens;
        let hPrev = yDensSeir * context.I[t-1];
        line(xPrev, y - hPrev, x, y - h)
        circle(x, y - h, 2, 2);
    ...
    stroke(colorR)
    for (let t = 1; t < context.R.length; t++) {

        x = leftX + t * xDens;
        let h = yDensSeir * context.R[t];
        let xPrev = leftX + (t-1) * xDens;
        let hPrev = yDensSeir * context.R[t-1];
        line(xPrev, y - hPrev, x, y - h)
        circle(x, y - h, 2, 2);
    ...
    stroke(colorD)
    for (let t = 1; t < context.D.length; t++) {

        x = leftX + t * xDens;
        let h = yDensSeir * context.D[t];
        let xPrev = leftX + (t-1) * xDens;
        let hPrev = yDensSeir * context.D[t-1];
        line(xPrev, y - hPrev, x, y - h)
        circle(x, y - h, 2, 2);
...
function sizeDensities() {
    ...
    yDens = 8 * graphAreaHeight / context.N[0];
