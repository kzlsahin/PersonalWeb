...
* here we define a Results class that will hold the result data,

* and has some methods to present data
...
let Results = {
    ...
    // inserting a new table for the results to be listed
    ...
    filterResults : function(){
        ...
        /*
        * this method is called when the filter button pressed
        * This method filters the results by summoning similar formats
        * and filters the formats that has a few entries
        */
        ...
        */
        ...
        let outputCopy = deepCopyObject(output);
        
        for(let markaObj of outputCopy){
            
            //marka loop
            
            // revise the status of information block
            
            document.getElementById("status").innerHTML = "filtered: " + Math.floor(((status + 0.5) / outputCopy.length) * 100) + " %";
            
            let label = markaObj.markaAdi;
            
            //we do not need serial numbers and motorTypes datas
            ...
            //lets push new marka object to filtereddata
            
            filteredMarkas.push({markaAdi: label, formats:[]});
            
            // formats data will be added at he end of loop
            ...
            for(let thisFormat of formatsData){
                ...
                //lets look at other formats and compere this format with them
                if( otherFormats != []){
            ...
            // filteredMarkas = [...{markaAdi: label, formats:[]}...]
            // now formatas array of objects will be added to last element of the filteredMarkas array
...
// we are to track the mouse position
...
// a method to remove a member from an array and returns as a new array

// forexample: arr = removeFromArray(arr, member)
...
// trims the format data and removes special characters
...
//downloads the filtered results as a .csv file
...
function exportFilteredResultsAsCSV(){
    ...
    if(filteredResult != null ){
        ...
        let markaList = deepCopyObject(filteredResult.markaDict);
...
const runFilteringProcess = () => {
    ...
    setTimeout(Results.filterResults, 50);
...
const openInformBlock = () => {
    
   document.getElementById("informationDiv").style.display = "block";
        
    document.getElementById("information").innerHTML = "Filtering... Please Wait..."; 
    
}

//never forget deepcopy again!
